...
const CreateCafePage = () => {
  ...
  const [rooms, setRooms] = useState([
    { roomType: '', systems: [{ systemType: '', count: '', specs: '', pricePerHour: '' }] },
  ]);
  ...
  // --- Handlers for form data (logic remains the same) ---
          ...
          <CardContent sx={{ p: 4 }}>
            ...
            <Box component="form" onSubmit={handleSubmit}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12}>
                  <TextField fullWidth label="Cafe Name" name="name" value={formData.name} onChange={handleChange} required />
                </Grid>
                <Grid item xs={12}>
                  <TextField fullWidth label="Full Address" name="address" value={formData.address} onChange={handleChange} required />
                </Grid>
                <Grid item xs={6} sm={3}>
                  <TextField fullWidth label="Opening Time" name="openingTime" type="time" value={formData.openingTime} onChange={handleChange} InputLabelProps={{ shrink: true }} required />
                </Grid>
                <Grid item xs={6} sm={3}>
                  <TextField fullWidth label="Closing Time" name="closingTime" type="time" value={formData.closingTime} onChange={handleChange} InputLabelProps={{ shrink: true }} required />
                </Grid>
                <Grid item xs={6} sm={3}>
                  <TextField fullWidth label="Longitude (Temp)" name="longitude" type="number" value={formData.longitude} onChange={handleChange} required />
                </Grid>
                <Grid item xs={6} sm={3}>
                  <TextField fullWidth label="Latitude (Temp)" name="latitude" type="number" value={formData.latitude} onChange={handleChange} required />
                </Grid>
              </Grid>
                ...
                <Card key={roomIndex} variant="outlined" sx={{ mb: 2, p: 2 }}>
                  ...
                  {room.systems.map((system, systemIndex) => (
                    <Grid container spacing={1} key={systemIndex} sx={{ mb: 1, alignItems: 'center' }}>
                      ...
                      <Grid item xs={6} sm={2}>
                        <IconButton color="error" onClick={() => removeSystem(roomIndex, systemIndex)}>
              ...
              <Button variant="outlined" startIcon={<AddCircleOutlineIcon />} onClick={addRoom} sx={{ mb: 2 }}>
                Add Another Room
              </Button>
